...
public:
    ...
    esp_err_t draw16bitbergbbitmap(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t *color_data);
    void fillScreen(uint16_t color);
    void te_on();
    void te_off();
    uint16_t width();
    uint16_t height();
    esp_err_t register_event_callbacks(const esp_lcd_dpi_panel_event_callbacks_t *cbs, void *user_ctx);
