...
bool UIManager::init() {
    ...
    lv_obj_t* main_page = lv_obj_create(screen);
    ...
    lv_obj_set_style_pad_hor(main_page, 0, LV_PART_MAIN);
    lv_obj_set_style_pad_ver(main_page, 0, LV_PART_MAIN);
    ...
    lv_obj_set_style_pad_gap(main_page, 0, LV_PART_MAIN);

    // Create status bar (first in flex layout)
    status_bar_ = new StatusBar();
    if (!status_bar_->create(main_page)) {
        delete status_bar_;
        status_bar_ = nullptr;
        return false;
    }
    ...
    if (!main_screen_->create(main_page)) {
        delete main_screen_;
